...
export function QuizPage() {
  ...
  const inProgressAttempts = useMemo(() => attempts.filter((attempt) => !attempt.finishedAt), [attempts]);
  ...
  const continueAttempt = async (attemptId: string) => {
    ...
    const nextIndex = attempt.questionIds.findIndex((questionId) => !attempt.answers[questionId]);
    setActiveAttemptId(attempt.id);
      ...
      <div className="h-full overflow-y-auto retro-scrollbar paper-page p-5 md:p-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex justify-between gap-3 flex-wrap items-start mb-5">
            ...
            <div className="flex items-center gap-2"><RetroButton variant="ghost" onClick={saveAndExit} icon={<Save size={15} />}>salvar e sair</RetroButton><RetroButton variant="primary" onClick={finishCurrent} icon={<CheckCircle2 size={15} />}>finalizar e corrigir</RetroButton></div>
          </div>
  ...
  if (screen === "result" && resultAttempt) {
    ...
    return (
      <div className="h-full overflow-y-auto retro-scrollbar paper-page p-5 md:p-8"><div className="max-w-4xl mx-auto">
        <div className="flex justify-between gap-3 flex-wrap"><div><p className="text-retro-comment text-[13px]">resultado · {resultAttempt.title}</p><h1 className="text-2xl font-bold text-retro-text">Simulado corrigido</h1></div><div className="flex items-center gap-2">{unanswered > 0 && <RetroButton onClick={() => void continueAttempt(resultAttempt.id)} icon={<Target size={15} />}>continuar simulado</RetroButton>}<RetroButton variant="primary" onClick={() => setScreen("home")} icon={<RotateCcw size={15} />}>voltar aos simulados</RetroButton></div></div>
